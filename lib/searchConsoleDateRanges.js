...
/** Search Console performance data is usually complete through ~2–3 days before “today”. */
...
export function getComparePresetRanges(presetId, ref = new Date()) {
  ...
  switch (presetId) {
    case "c24h_prev": {
      const p = getDateRangeForPresetId("24h", ref);
    ...
    case "c24h_wow": {
      ...
      const y1 = formatYMD(yday);
      const y8 = addCalendarDays(y1, -7);
      if (!y1 || !y8) return null;
      return {
        primary: { startDate: y1, endDate: y1 },
        compare: { startDate: y8, endDate: y8 },
      };
    }
    case "c7d_prev": {
